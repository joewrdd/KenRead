...
export default function LatestPage() {
  ...
  const fetchManga = async (currentOffset: number) => {
    try {
      setIsLoading(true);
      const response = await mangadexApi.getLatestManga(limit, currentOffset);

      if (response.data && response.data.length > 0) {
        if (currentOffset === 0) {
          setLatestManga(response.data);
        } else {
          setLatestManga((prev) => [...prev, ...response.data]);
        }
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold">Latest Manga Updates</h1>
      ...
      {latestManga.length === 0 && !isLoading && (
